...
reponame="metabase"
APP_VERSION="v0.50.18.1"

# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-metabase container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-metabase; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-metabase -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/metabase/metabase:${APP_VERSION}" \
    "${container}"
# Commit the image
